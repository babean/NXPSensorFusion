...
bool NXPMotionSense::begin()
{
	...
	#if defined (PARTICLE)
		EEPROM.get(NXP_MOTION_CAL_EEADDR + i, buf[i]);
	#else
...
bool NXPMotionSense::writeCalibration(const void *data)
{
	...
	for (i=0; i < NXP_MOTION_CAL_SIZE; i++) {
	#if defined (PARTICLE)
		EEPROM.put(NXP_MOTION_CAL_EEADDR + i, p[i]);
	#else
	...
	for (i=0; i < NXP_MOTION_CAL_SIZE; i++) {
	#if defined (PARTICLE)
		EEPROM.get(NXP_MOTION_CAL_EEADDR + i,x);
